...
export function registerThaw(program: Command): void {
  program
    .command("thaw")
    .description("Thaw a frozen token account (requires FreezeAuth role)")
    .requiredOption("--config <pubkey>", "Stablecoin config account address")
    .requiredOption(
      "--address <pubkey>",
      "Wallet address whose token account will be thawed"
    )
    .action(async (opts) => {
      ...
      try {
        ...
        const sig = await stablecoin.thawAccount(ctx.wallet, tokenAccount);
